...
def main(build_name, build_version, build_path, fleet_name, aws_region, environment):
    build_id = upload_build(build_name, build_version, build_path, aws_region)
    log_step(f"Build ID: {build_id}")

    while not build_ready_or_failed(build_id, aws_region):
    ...
    fleet_id = create_fleet(
        fleet_name, build_id, launch_path, PROJECT_NAME, aws_region, environment
    )
    log_step(f"Received Fleet ID: {fleet_id}")

    write_file(
        os.path.join(os.getcwd(), RESULT_FILE_NAME),
        fleet_id
    )
...
def build_ready_or_failed(build_id, aws_region):
    result = aws_cli(
        [
            "gamelift",
            "describe-build",
    ...
    if status == "ERROR":
        log_step("Build Uploaded with status: ERROR")
        sys.exit(2)
...
def create_fleet(name, build_id, launch_path, project_name, region, environment):
    ...
    result = aws_cli(
        [
            ...
            "--locations", json.dumps(get_locations(region)),
...
def write_file(file_path, contents):
    log_step(f"Writing {contents} to {file_path}.")
    f = open(file_path, "w")
    f.writelines(contents)
    f.close()
    log_step(f"Successfully wrote {file_path} to disk.")
